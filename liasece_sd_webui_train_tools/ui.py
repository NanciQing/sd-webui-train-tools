...
def gr_show(visible=True):
    """
    在 Gradio 4.x 中，返回 gr.update(visible=...)
    即可控制显隐。
    """
    ...
    with gr.Blocks(css="button {background-color: #2171f1}") as train_tools:
        ...
        with gr.Row(visible=False) as gr_project_version_dateset_row:
            ...
            with gr.Column():
                ...
                with gr.Box():
                    ...
                    with gr.Row():
                        gr_project_version_dataset_gallery = gr.Gallery(value=None, label='Output', show_label=False, elem_id=f"gr_project_version_dataset_gallery").style(columns=4)
                    with gr.Row():
            ...
            with gr.Column():
                ...
                with gr.Box():
                    # UI: upload dataset
                    with gr.Row():
                        # 旧版中 tool="sketch" 已经不再适用于 Files 组件，此处去掉
                        input_train_data_set_files = gr.Files(interactive=True, label="Upload Dataset")
                    with gr.Box():
            ...
            with gr.Box():
                with gr.Row():
                    with gr.Column():
                        with gr.Row():
                            ...
                            train_base_model = gr.Dropdown(
                                ...
                                value=tiles[0] if len(tiles) > 0 else "",
                                choices=tiles,
                                interactive=True,
                                elem_id="train_base_model"
                            )
                    ...
                    with gr.Column():
                        ...
                        sd_script_args = gr.Textbox(value="", label="Append or override the sd_script args.", elem_id="sd_script_args", interactive=True)
                    with gr.Column():
                        ...
                        train_optimizer_type = gr.Dropdown(
                            label="Optimizer type(Multi-select)",
                            value=["Lion"],
                            choices=["Adam", "AdamW", "AdamW8bit", "Lion", "SGDNesterov", "SGDNesterov8bit", "DAdaptation", "AdaFactor"],
                            multiselect=True,
                            interactive=True,
                            elem_id="train_optimizer_type"
                        )
                        train_mixed_precision = gr.Dropdown(
                            label="Mixed precision (If your graphics card supports bf16 better)",
                            value="fp16",
                            choices=["fp16", "bf16"],
                            interactive=True,
                            elem_id="train_mixed_precision"
                        )
                with gr.Row():
        ...
        with gr.Box(visible=False) as preview_box:
            ...
            with gr.Box():
                # UI: preview config
                with gr.Row():
                    preview_txt2img_prompt = gr.Textbox(
                        label='Prompt',
                        value="best quality,Amazing,finely detail,extremely detailed CG unity 8k wallpaper",
                        placeholder="Prompt",
                        elem_id="preview_txt2img_prompt",
                        interactive=True,
                        lines=3
                    )
                with gr.Row():
                    preview_txt2img_negative_prompt = gr.Textbox(
                        label='Negative Prompt',
                        value="worst quality, low quality, normal quality",
                        placeholder="Negative Prompt",
                        elem_id="preview_txt2img_negative_prompt",
                        interactive=True,
                        lines=3
                    )
                with gr.Row():
                    ...
                    with gr.Column():
                        preview_sampling_method = gr.Dropdown(
                            label='Sampling method',
                            choices=[x.name for x in sd_samplers.samplers],
                            value=[sd_samplers.samplers[0].name],
                            multiselect=True,
                            elem_id="preview_sampling_method",
                            interactive=True
                        )
            ...
            max_list_checkpoint = 10  # 原脚本里没显示定义，可根据原项目适当调整
            for i in range(0, max_list_checkpoint):
                with gr.Box(visible=False) as train_checkpoint_row:
                    with gr.Row():
                        ...
                        with gr.Column(scale=2):
                            with gr.Row():
                                g = gr.Gallery(value=None, visible=False, label='Output', show_label=False, elem_id=f"train_checkpoint_txt2txt_preview_gallery").style(columns=4)
                                train_checkpoint_txt2txt_preview_gallery_list.append(g)
                                s = gr.Image(value=None, visible=False, label='Output', show_label=False, elem_id=f"txt2txt_image", interactive=False)
                                train_checkpoint_txt2txt_preview_single_image_list.append(s)
                            with gr.Row():
                                train_checkpoint_txt2txt_preview_btn_log_list.append(gr.HTML(elem_id=f'html_log_Preview'))
                            with gr.Row():
                                c = gr.Checkbox(label="Delete preview images before generate", value=True, interactive=True)
                                train_checkpoint_txt2txt_preview_delete_before_generate_list.append(c)
                                b = gr.Button(value="Generate preview", variant="primary")
                                train_checkpoint_txt2txt_preview_btn_list.append(b)
        ...
        def dataset_outputs():
            return [
                gr_project_version_dateset_row,
                train_row,
                preview_box,
                gr_project_version_dataset_gallery,
                gr_project_version_dataset_label
            ]
        ...
        gr_project_dropdown.change(
            ...
            outputs=[
                gr_project_dropdown,
                gr_project_version_row,
                gr_version_dropdown
            ] + dataset_outputs() + trains_area_outputs() + all_config_inputs(),
        )
        ...
        update_dataset_btn.click(
            ...
            # show_progress=False  <-- 在 Gradio 4.x 里去掉
        )
        # train
        ...
        train_begin_btn.click(
            ...
            inputs=[
                ...
                gr_project_dropdown,
                gr_version_dropdown
            ] + train_config_inputs() + preview_config_inputs(),
